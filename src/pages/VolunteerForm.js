...
const VolunteerForm = () => {
    ...
    const formatPhoneNumber = (value) => {
        ...
        const parts = [];
        if (cleaned.length >= 3) parts.push(cleaned.slice(0, 3));
        if (cleaned.length >= 6) parts.push(cleaned.slice(3, 6));
        if (cleaned.length > 6) parts.push(cleaned.slice(6, 10));

        return parts.join('-'); // Join parts with dashes
    };
    ...
    return (
        <div style={styles.pageContainer}>
            ...
            <header style={styles.header}>
                <button style={styles.headerButton} onClick={goBack}>Back</button>
            </header>
